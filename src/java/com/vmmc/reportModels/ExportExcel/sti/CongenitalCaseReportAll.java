 ...
 * @author devoop
 */
@WebServlet(name = "CongenitalCaseReportAll", urlPatterns = {"/CongenitalCaseReportAll"})
public class CongenitalCaseReportAll extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CongenitalCaseReportAll</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CongenitalCaseReportAll at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
          RequestDispatcher rd = request.getRequestDispatcher("export/CSReport.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
      ...
      String startDatef = request.getParameter("txtRptStartDate");
       ...
       // List<Sti> epmtylist = new ArrayList();
        if (!cs.isEmpty()) {
            for (StiCs currentApp : cs) {
                if (currentApp.getDateofReport().compareTo(startDate) >= 0 && currentApp.getDateofReport().compareTo(endDate) <= 0) {
                    cs.add(currentApp);
                }
            }
        }
