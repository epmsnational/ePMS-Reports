 ...
 * @author Trevor
 */
@WebServlet(name = "AllNotifications", urlPatterns = {"/AllNotifications"})
public class AllNotifications extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AllNotifications</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AllNotifications at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Cbs> cbsList = new cbsablePatientImpl().getAllPatients();//change to return all Vmmcs
        List<Cbs> tempList = new ArrayList<>();
        for (Cbs c : cbsList) {
            if (c.getFirstname() != null) {
                tempList.add(c);
            }
        }
        request.setAttribute("cbss", tempList);
        RequestDispatcher disp = request.getRequestDispatcher("cbs/AllNotifications.jsp");
