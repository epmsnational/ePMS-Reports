 ...
 * @author Mohcc-ATP
 */
public class AllSTICs extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AllSTICs</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AllSTICs at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<StiCs> listStics = new StiCSableImpl().getAllCongenitalSyph();//change to return all Vmmcs
        List<StiCs> tempList = new ArrayList<>();
        for (StiCs p : listStics) {
            if (p.getStinumber()!= null) {
                tempList.add(p);
            }
        }
        request.setAttribute("patients", tempList);
       // request.setAttribute("patients", listStics);
        RequestDispatcher disp = request.getRequestDispatcher("stics/AllCongenitalCaseNotifications.jsp");
