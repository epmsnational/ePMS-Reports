 ...
 * @author devoop
 */
public class DirectAdults extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DirectAdults</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DirectAdults at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        /// Step 4: Risks past 12 months
        String cbssexualabused = request.getParameter("victimissexualabused");
        String historyofsti = request.getParameter("ishistoryofsti");
        String cbsblodtrsfusion = request.getParameter("isbloodtransfusion");
        String cbsinjectedheroin = request.getParameter("isinjectedheroin");
        String cbsmedicalinjections = request.getParameter("ismedicalinjections");
        String cbsbeeninprison = request.getParameter("iseverbeeninprison");
        String cbshavesexformoney = request.getParameter("ishavesexformoney");
        String cbssexwithsexworker = request.getParameter("issexwithsexworker");
        String cbshaveuprotextedsexman = request.getParameter("ishaveunprotectedsexmale");
        String cbshaveuprotextedsexwoman = request.getParameter("ishaveunprotectedsexfemale");
        String cbsunprotectedsexwithoutcondom = request.getParameter("unprotectedsexwithoutcondom");
        String cbstatooedwithunsterilized = request.getParameter("istatooedwithunsterilized");
        String cbstxtotherroute = request.getParameter("txtotherroute");
        String cbsprobabletransimission = request.getParameter("txtprobabletransimission");
        String cbsMostRecentNegative = request.getParameter("txtCBSrecentNegativeResult");
        String cbsMostRecentNegativeMonths = request.getParameter("txtCBSrecentNegativeResultMonths");
        String cbsEverBeenOnPrep = request.getParameter("txtcbsbeenonPrEP");
        String cbsEverBeenOnAnyArt = request.getParameter("txtcbsbeenonanyART");
        String cbsCurrentlyOnArt = request.getParameter("txtcbscurrentlyonart");

        //Step 5 :  Recency Testing
        String cbsRapidTestRecency = request.getParameter("txtcbsrtri");
        String cbsTesterNameRecency = request.getParameter("txtCBSrecenttestername");
        String cbsTestDateRecency = request.getParameter("txtCBSrecenttestdate");
        String cbsResultRecency = request.getParameter("txtcbsrecenttestresult");
        String cbsReasonNotDone = request.getParameter("txtcbsrecenttestreason");

        //Step 6 :  HIV Clinical Info
        ...
        try {
            ...
            Date parseHivNegative = formatter.parse(cbsMostRecentNegative + " 00:00:00");
            cbsHivNegative = new java.sql.Timestamp(parseHivNegative.getTime());
        ...
        //step4
        cb.setSexualabused(cbssexualabused);
        cb.setSexWithSexWorker(cbssexwithsexworker);
        cb.setUnProtectedSexWithFemale(cbshaveuprotextedsexwoman);
        cb.setUnProtectedSexWithMale(cbshaveuprotextedsexman);
        cb.setInjectables(cbsinjectedheroin);
        cb.setStihistory(historyofsti);
        cb.setBeenPrison(cbsbeeninprison);
        cb.setExchangedSexMoney(cbshavesexformoney);
        cb.setRecBloodTranfusion(cbsblodtrsfusion);
        cb.setRecMedicalInjection(cbsmedicalinjections);
        cb.setTatooedwithunsterilized(cbstatooedwithunsterilized);
        cb.setUnprotectedsexnocondom(cbsunprotectedsexwithoutcondom);
        cb.setOtherrouteoftransmission(cbstxtotherroute);
        cb.setRecentPositive(cbsHivNegative);
        cb.setMonthsRecentPositive(cbsMostRecentNegativeMonths);
        cb.setEverBeenOnPrep(cbsEverBeenOnPrep);
        cb.setEverBeenOnAnyArtNotPrep(cbsEverBeenOnAnyArt);
        cb.setCurrentlyOnArt(cbsCurrentlyOnArt);
...
//        cbsdb.add(cb);
        if (cbsdb.add(cb) == 1) {
            ...
            Timestamp cbsRecencyTestDate = null;
            ...
            try {
                ...
                Date parsertridate = formatter.parse(cbsTestDateRecency + " 00:00:00");
                cbsRecencyTestDate = new java.sql.Timestamp(parsertridate.getTime());
            ...
            // Step 5
            cbsAdult.setRecencyTesting(cbsRapidTestRecency);
            if (cbsRapidTestRecency.equalsIgnoreCase("Done")) {
                cbsAdult.setRecencyTesterName(cbsTesterNameRecency);
                cbsAdult.setRecencyTestingDate(cbsRecencyTestDate);
                cbsAdult.setRecencyTestingResult(cbsResultRecency);
            } else {
                cbsAdult.setRecencyTestingNotDone(cbsReasonNotDone);
            }
